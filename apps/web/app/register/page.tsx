...
export default function RegisterPage() {
  ...
  const [role, setRole] = useState<Extract<UserRole, "CUSTOMER" | "STORE_OWNER">>("CUSTOMER");
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      router.push(user.role === "STORE_OWNER" ? "/stores/new" : "/");
    } catch (err) {
      ...
      <div className="hidden lg:flex lg:w-[45%] xl:w-[40%] bg-primary-900 relative flex-col justify-between p-12 text-white min-h-screen select-none">
        ...
        <div className="relative z-10 my-auto space-y-4 pr-4">
          <h2 className="text-3xl xl:text-4xl font-extrabold tracking-tight leading-tight">
            Junte-se à maior rede de petshops da sua região.
          </h2>
          <p className="text-primary-50/90 text-sm xl:text-base max-w-sm font-medium leading-relaxed">
            Crie sua conta para começar a comprar ou para gerenciar seu negócio e vender para milhares de clientes.
          </p>
        </div>
        ...
        <div className="w-full max-w-md space-y-6">
          ...
          <div className="grid grid-cols-2 gap-1 bg-zinc-100/80 p-1 rounded-xl">
            <button
              ...
              className={`rounded-lg py-2.5 px-4 text-center transition duration-200 select-none outline-none focus:outline-none cursor-pointer ${
                role === "CUSTOMER"
                  ? "bg-white text-primary-600 shadow-sm"
                  : "text-ink-muted hover:text-ink"
              }`}
            >
              <span className="block text-sm font-bold leading-none">Quero comprar</span>
              <span className={`block text-[10px] mt-1 font-medium ${
                role === "CUSTOMER" ? "text-ink-muted" : "text-ink-muted/60"
              }`}>Buscar petshops</span>
            </button>
            <button
              type="button"
            ...
            >
              <span className="block text-sm font-bold leading-none">Tenho petshop</span>
              <span className={`block text-[10px] mt-1 font-medium ${
                role === "STORE_OWNER" ? "text-ink-muted" : "text-ink-muted/60"
              }`}>Vender e gerenciar</span>
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-[11px] text-ink-muted">{PASSWORD_REQUIREMENTS_HINT}</p>
            </div>

            <div className="space-y-1.5">
              <label htmlFor="password-confirmation" className="text-sm font-semibold text-ink">
                Confirmar senha
              </label>
              <input
          ...
          <p className="text-center text-sm text-ink-muted">
            Já tem conta?{" "}
            <Link href="/login" className="font-semibold text-primary-600 hover:text-primary-700 hover:underline transition">
              Entrar
            </Link>
          </p>
        </div>
