...
| **Pedido** (C7) — ⏳ **parcialmente entregue** | ✅ Carrinho de uma loja (no cliente), **cotação** no servidor, validação de disponibilidade / área / loja ativa / horário, criação com `Idempotency-Key`, comissão por categoria com override e zero por indicação, snapshot, cancelamento pelo tutor, **auto-recusa por prazo vencido** e acompanhamento de status (`pd-15` — ver [`PEDIDO_E_CARRINHO`](../08-features/orders/PEDIDO_E_CARRINHO.md)); ⏳ aceitar, recusar, despachar, concluir e marcar item indisponível **existem no domínio e não têm rota** — dependem do `StoreScopeGuard` (`pd-16`); ⬜ substituição assistida (sem produtor); ⬜ pagamento (`pd-17`) | MVP_SCOPE #6 |
