...
| Execução / hosting | **Railway** (API, landing, Postgres) + **Cloudflare** (DNS e Pages para o app web) + **Registro.br** (domínio) | ✅ **Decidido em 13/09/2026** ([ADR-0020](../06-decisions/ADR/0020-hosting-do-piloto-railway-e-cloudflare.md)) pelo critério de custo mínimo com zero operação de banco: ~US$ 10-15/mês, Hobby, região US East. Migrations por pre-deploy command com falha fechada; produção acompanha `master`. ⚠️ **Passos de deploy fechados na `pd-19`** (`railway.json` por serviço, migrations por pre-deploy, `_redirects`/`_headers` do app web, DNS) — ver [`DEPLOYMENT`](../03-engineering/DEPLOYMENT.md) §"Passos de deploy". A conta só se cria quando a publicação estiver a dias (E8). Sem PITR e com dados nos EUA, aceitos com gatilhos nomeados; a saída é o Fly.io em São Paulo. |
| Observabilidade | **OpenTelemetry (SDK instrumentado) → Grafana Cloud Free** | Traces e métricas saindo por OTLP desde a `pd-04`; logs estruturados em stdout com `trace_id`. Instrumentações: HTTP, Express, pino, Prisma. Ligado por `OTEL_EXPORTER_OTLP_ENDPOINT`, desligado por padrão. Coletor local em dev (`npm run otel:up`). ✅ **Destino escolhido em 13/09/2026: Grafana Cloud, plano gratuito** ([ADR-0021](../06-decisions/ADR/0021-destino-da-telemetria-do-piloto.md)) — 3 usuários e 14 dias de retenção foram o que decidiu, não preço. Três variáveis no serviço da API, zero linha de código. ⚠️ **Logs seguem no Railway** (7 dias), com gatilho próprio; `/api/v1/health` saiu dos traces. |
