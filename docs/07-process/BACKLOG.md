...
| `pd-14` | `tutors` — perfil, endereço padrão e pets (capacidade 2). É o que o pedido precisa para ter destino de entrega. **Inclui a tela de cadastro**, que a `pd-13` deixou de fora de propósito (ADR-0012, A10): criar conta é o primeiro passo da **J1**, e ela continua com endereço e pet — separá-los daria uma tela de cadastro que não leva a lugar nenhum | nenhuma |
| `pd-15` | `orders` — carrinho, pedido, máquina de estados, **sem pagamento**: o pedido para em `PLACED`. ✅ **As quatro decisões de modelagem que a bloqueavam foram tomadas em 12/09/2026** ([ADR-0014](../06-decisions/ADR/0014-ciclo-do-dinheiro-no-pedido.md)): prazo de aceite de 15 min contado só em horário de funcionamento, agenda semanal da loja, ajuste por item em falta e cancelamento livre até o aceite. ⚠️ Traz junto a **agenda semanal** (`Store`) e a entidade **`Refund`**, ainda que a devolução só passe a funcionar de verdade com o PSP da `pd-17` | nenhuma |
