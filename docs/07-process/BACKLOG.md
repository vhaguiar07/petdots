...
| **Logs não chegam ao backend de telemetria** | Desde a `pd-04` os logs carregam `trace_id`/`span_id` e são correlacionáveis, mas seguem **só em stdout** — nada os envia a lugar nenhum. Enviar exige decidir o deploy (agente coletor × transport do pino), e não há deploy. **Trabalho:** decidir junto com o `DEPLOYMENT` | ADR-0006 (alternativas consideradas), 08/09/2026 |
...
| **Sem teste automatizado provando que spans existem** | **Medido em 08/09/2026:** o sentinela de span **não funciona sob Jest** — o Jest substitui o sistema de módulos por um registry próprio, e o hook de `require` do OTel engancha o `Module._load` real; dentro de um spec, `require('http').createServer` volta sem patch (`__wrapped` undefined). A cobertura automatizada existe para o que é código próprio (`resolveOtelConfig`, `signalUrl`, `shouldIgnoreRequest` — 20 testes), e a existência de span de verdade foi provada **manualmente** contra o coletor local. **Risco:** uma regressão na fiação (ex.: alguém reordenar o primeiro import do `main.ts`) não é pega por teste. **Trabalho:** avaliar teste fora do Jest (script Node de verdade contra o coletor, no CI) ou migrar essa suíte para Vitest | Tentativa de escrever o sentinela na `pd-04`, 08/09/2026 |
