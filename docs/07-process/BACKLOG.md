...
| 6 | **Publicar** (landing + API + Postgres + app web + domínio). ✅ **O provedor deixou de ser pergunta em 13/09/2026:** Railway (API, landing, Postgres) + Cloudflare (DNS e Pages para o app web) + Registro.br ([ADR-0020](../06-decisions/ADR/0020-hosting-do-piloto-railway-e-cloudflare.md)) | Critério do [`DEPLOYMENT`](../03-engineering/DEPLOYMENT.md) fechado pelo ADR-0020; os passos de deploy são a tarefa de publicação; dispara os itens de rate limit, de observabilidade e do export estático na vigilância | Envolve conta e cartão — 🔴 **e nada é pago antes de a tarefa de publicação começar** (ADR-0020, E8): a conta no Railway se cria quando a publicação estiver a dias, para o trial de 30 dias cobrir a montagem. O que só você faz: criar a conta, colar o `JWT_SECRET`, trocar os nameservers de `petdots.com.br` no Registro.br para o Cloudflare, promover `develop` → `master` (a produção acompanha `master`, E7). ⚠️ **É o que trava o smoke test hoje** — a landing existe e passa no CI, mas roda só em `localhost`. Publica-se **sem seed** e com a `/precos` fora do menu até o censo (E9) |
