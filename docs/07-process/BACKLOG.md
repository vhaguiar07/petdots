...
| **`StoreScopeGuard` e autorização fina não existem — gatilho: `StoreMember` no schema** | O `SECURITY` chama o escopo por instância de "defesa central" e fixa a invariante **0 acesso a pedido ou preço de outra loja**; ela **não está implementada** ([ADR-0011](../06-decisions/ADR/0011-autenticacao-propria-antes-da-escrita.md), A2). 🔶 **Eram dois pré-requisitos; sobrou um.** (a) ✅ A distinção `OWNER` × `OPERATOR` **foi fechada em 12/09/2026** ([ADR-0013](../06-decisions/ADR/0013-papeis-de-loja-owner-e-operator.md)) — a tabela de permissões está no `SECURITY`, e o guard já sabe o que negar. (b) ⏳ `StoreMember` continua fora do `schema.prisma`, e nasce com o painel do lojista (`pd-16`), junto da migration e do próprio guard. ⚠️ **Enquanto isso, nenhuma escrita de dado de loja pode ser aberta**: sem esse guard, a loja A altera a oferta da loja B | `pd-12`, 12/09/2026; pré-requisito (a) resolvido em 12/09/2026 |
...
| **`pd-16`** ← **próxima** | Painel do lojista + `StoreMember` + `StoreScopeGuard`. ✅ **Destravada em 12/09/2026:** a tabela de permissões está decidida no [ADR-0013](../06-decisions/ADR/0013-papeis-de-loja-owner-e-operator.md) e é insumo pronto deste plano. ✅ **E muito mais barata depois da `pd-15`:** os casos de uso de **aceite, recusa, despacho, entrega e item indisponível já existem no domínio de `orders`**, puros e testados — inclusive a regra de que o último item indisponível cancela o pedido com devolução total. A `pd-16` liga **controllers + guard** a funções que já têm forma, e as transições da loja herdam a auditoria sem código novo. Falta também a tela que deixa o `OWNER` editar a **agenda semanal**, hoje escrita só pelo seed | ✅ **nenhuma** — a decisão `OWNER` × `OPERATOR` que a travava foi tomada |
| `pd-17` | `payments` — PSP, split, repasse e a **devolução** que o ADR-0014 exige. **A `pd-15` deixou quatro coisas nomeadas para cá:** (a) o **estado anterior ao pagamento** e a mudança de *quando* o pedido é criado (ADR-0017, A1 — `PlaceOrderUseCase` é parcialmente reescrito); (b) a **FK de `refunds.payment_id`**, hoje nula e sem referência; (c) **o que fazer com os `Refund` de pedidos que nunca foram cobrados**, que a `pd-15` gerou em desenvolvimento; (d) a **biblioteca de scheduling**, se quiser uma — é no segundo job (conciliação diária) que a decisão passa a ter duas necessidades reais para se apoiar | 🔴 **conta no PSP** (item 6/manual) + três decisões de modelagem que seguem abertas (notificação transacional, obrigações fiscais do split, extrato de repasse). ✅ O **estorno** deixou de ser uma delas: está decidido no [ADR-0014](../06-decisions/ADR/0014-ciclo-do-dinheiro-no-pedido.md) |
...
das quatro que **não** destrava outra. A `pd-16` tira o pedido do limbo — hoje
ninguém pode aceitar, e todo pedido expira —, e a `pd-17` fecha o ciclo do
dinheiro. Vários endereços melhora o que já funciona; as duas anteriores fazem
