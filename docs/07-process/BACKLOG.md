...
| Item | Detalhe | Origem |
|---|---|---|
| 🔴 **Aplicar a identidade visual nos três frontends** | A marca está **decidida e documentada** desde 14/09/2026 — escala Biscoito, Gabarito, o logo com três pontos e o medidor ([ADR-0022](../06-decisions/ADR/0022-identidade-visual-biscoito.md), [`BRAND_IDENTITY`](../00-foundation/BRAND_IDENTITY.md)) — e **nada está aplicado**. Hoje `apps/app` usa o vocabulário neutro do spike (`src/ui/theme.ts`, cores escolhidas por contraste) e `apps/landing` tem uma paleta própria de papel e verde: **são duas marcas acidentais**. **Trabalho:** um arquivo de tokens compartilhado entre os dois frontends — hoje não existe, e é por isso que divergiram —, o logo em **SVG**, favicon e ícones de app nas resoluções de loja, as **cores de estado** calibradas contra a escala, e o adesivo de vitrine com QR. ⚠️ **Não depende de nada externo**, e é o que separa "parece HTML puro" de produto que se leva a campo. ⚠️ O tema do app hoje é consumido por 8 componentes-envelope; trocar os valores é barato, trocar a forma não | Decisão do Victor, 14/09/2026, ao fim de seis rodadas de exploração visual |
