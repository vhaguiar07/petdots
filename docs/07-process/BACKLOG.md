...
| 4 | **Definir o canal de contato do aviso de privacidade** (LGPD) — ✅ **encaminhado em 13/09/2026:** vira a página `/privacidade` da `pd-19`, com o texto já rascunhado em [`AVISO_DE_PRIVACIDADE`](../01-product/AVISO_DE_PRIVACIDADE.md). ⏳ **Pendência sua, anotada em 13/09/2026: criar o e-mail `contato@petdots.com.br`** (ou definir para qual caixa sua ele é encaminhado, premissa A2 do rascunho). *"Vou fazer depois"* — a `pd-19` só precisa dele no dia de ligar o DNS | Constante `PRIVACY_CONTACT` em `apps/landing/src/content/privacy.ts`; rascunho em `docs/01-product/AVISO_DE_PRIVACIDADE.md` | O valor hoje é `contato@petdots.com.br (a definir)`, **deliberadamente marcado como provisório**. ⚠️ Não publicar a landing com ele assim — é o canal que a LGPD manda oferecer ao titular. O endereço `contato@` não existe como caixa: com o DNS no Cloudflare (ADR-0020), o Email Routing encaminha para uma caixa existente sem custo |
...
| 6 | **Publicar** (landing + API + Postgres + app web + domínio) — **é a `pd-19`**, ver a sequência acordada. ⏳ **Conta no Railway pendente: você cria quando a publicação estiver a dias**, não antes (13/09/2026). ✅ **O provedor deixou de ser pergunta em 13/09/2026:** Railway (API, landing, Postgres) + Cloudflare (DNS e Pages para o app web) + Registro.br ([ADR-0020](../06-decisions/ADR/0020-hosting-do-piloto-railway-e-cloudflare.md)) | Critério do [`DEPLOYMENT`](../03-engineering/DEPLOYMENT.md) fechado pelo ADR-0020; os passos de deploy são a tarefa de publicação; dispara os itens de rate limit, de observabilidade e do export estático na vigilância | Envolve conta e cartão — 🔴 **e nada é pago antes de a tarefa de publicação começar** (ADR-0020, E8): a conta no Railway se cria quando a publicação estiver a dias, para o trial de 30 dias cobrir a montagem. O que só você faz: criar a conta, colar o `JWT_SECRET`, trocar os nameservers de `petdots.com.br` no Registro.br para o Cloudflare, promover `develop` → `master` (a produção acompanha `master`, E7). ⚠️ **É o que trava o smoke test hoje** — a landing existe e passa no CI, mas roda só em `localhost`. Publica-se **sem seed** e com a `/precos` fora do menu até o censo (E9) |
...
| 11 | 🔴 **Acrescentar `CORS_ORIGINS=http://localhost:8081` ao `.env` local e reiniciar a API** — e, quando existir ambiente de deploy, a origem real do app publicado | Nasce aqui (`pd-13`). Documentado no [`DEVELOPMENT_GUIDE`](../03-engineering/DEVELOPMENT_GUIDE.md) e no `.env.example` | O `apps/app` chama a API **pelo navegador** (a landing a chama pelo servidor), então sem essa variável o browser bloqueia **todas** as chamadas — e **o sintoma engana**: o badge do topo diz "API: fora do ar" com a API perfeitamente de pé. A IA não escreve no seu `.env`, que é local e não versionado (precedente do item 9), e `nest start --watch` **não relê o `.env`**: a API precisa ser reiniciada por você |
| 12 | **Confirmar a base legal do cadastro e o texto do aviso de privacidade linkado em `/cadastro`** — ✅ **base legal aprovada pelo Victor em 13/09/2026** (conta é execução de contrato; lista de espera é consentimento). O destino do link é a `/privacidade` da `pd-19`; a frase nova está na Parte 2 de [`AVISO_DE_PRIVACIDADE`](../01-product/AVISO_DE_PRIVACIDADE.md). ⏳ **Falta a sua verificação do texto** | Nasce aqui (`pd-14`). Frase atual em `apps/app/src/screens/register-screen.tsx`: *"Ao criar a conta você concorda com o aviso de privacidade do PetDots."* | Decisão de produto e jurídica. A `pd-14` nasceu **sem checkbox de consentimento** por um motivo: a base legal de ter uma conta é a **execução do contrato**, não consentimento — diferente da lista de espera, onde o checkbox existe porque o contato posterior é que exige consentir (ADR-0015, D10). Confirme a leitura, e diga **para onde a frase deve apontar** — hoje ela não é um link, porque o aviso de privacidade só existe na landing |
...
| 15 | **Declarar no aviso de privacidade que os dados ficam em servidores nos Estados Unidos** — junto da revisão dos itens 4 e 12. ✅ **Rascunhado em 13/09/2026:** §3 de [`AVISO_DE_PRIVACIDADE`](../01-product/AVISO_DE_PRIVACIDADE.md). ⏳ **Falta a sua verificação**, e a premissa A5 (garantias contratuais dos provedores) pede leitura jurídica | Nasce aqui (13/09/2026), a partir do [ADR-0020](../06-decisions/ADR/0020-hosting-do-piloto-railway-e-cloudflare.md) E10. Rascunho em `docs/01-product/AVISO_DE_PRIVACIDADE.md`; constante provisória em `apps/landing/src/content/privacy.ts` | Hospedar no Railway (Virgínia) é **transferência internacional de dados** pelo art. 33 da LGPD, regulamentada pela Resolução CD/ANPD nº 19/2024. A IA redige a frase quando você mandar; **a leitura jurídica é sua** — e se ela concluir que residência no Brasil é obrigatória, o gatilho (b) do ADR-0020 E11 dispara e a saída nomeada é o Fly.io em São Paulo, a US$ 38 a mais por mês de banco |
...
| 🔴 **O export estático do app gera `[param].html` e dá 404 no F5 de toda rota dinâmica em host estático puro — gatilho: deploy do app web** | **Medido em 13/09/2026** no `apps/app/dist` local: com `web.output: "static"`, o `expo export` emite `painel/[storeId].html`, `painel/[storeId]/pedidos/[orderId].html`, `pedidos/[orderId].html`, `conta/pets/[petId].html`, `loja/[storeId].html` e `precos/[productSlug].html` — o nome literal do parâmetro. Um host que só serve arquivos responde 404 a `/painel/abc`. A documentação do Expo confirma: rotas dinâmicas *"will not work out of the box"* no output estático. Em `expo start` o problema não aparece, porque o dev server resolve a rota. **Decidido no [ADR-0020](../06-decisions/ADR/0020-hosting-do-piloto-railway-e-cloudflare.md) E5:** arquivo `_redirects` em `apps/app/public/` reescrevendo cada rota dinâmica para o seu `[param].html` com status 200 (o Cloudflare Pages aceita placeholder e 200; o Expo copia `public/` para o `dist`), ou, se frágil, `web.output: "single"` com regra catch-all. **Trabalho:** na tarefa de publicação, escolher entre as duas com o export na mão e provar com F5 em cada rota dinâmica | ADR-0020, 13/09/2026 |
...
| **Web do `apps/app` sem CSP — gatilho: deploy do app web** | A sessão do cliente fica no `localStorage` no navegador ([ADR-0012](../06-decisions/ADR/0012-sessao-do-cliente-universal-e-guards-globais.md), P4/A4), e o risco aceito é **XSS**: um script rodando na origem lê access e refresh token. **O que já limita:** o React Native Web escapa todo texto que renderiza (não há `dangerouslySetInnerHTML` com dado de usuário — o único uso é o CSS estático de `+html.tsx`), e a rotação do refresh faz uma cópia roubada morrer na próxima renovação legítima. **O que falta:** uma Content-Security-Policy servida junto do export estático, que só pode ser definida quando existir onde publicá-lo. **Não é acionável hoje** — não há deploy | `pd-13`, 12/09/2026 |
...
| **`GET /postal-codes/{cep}` sem rate limit — gatilho: deploy público** | A rota criada na `pd-14` ([ADR-0016](../06-decisions/ADR/0016-diretorio-de-ceps-atras-da-nossa-api.md)) repassa consultas a um diretório de terceiro gratuito e sem SLA. Ela é **autenticada**, o que já limita o abuso a quem tem conta — e foi por isso que nasceu fechada —, mas não o elimina: uma conta pode varrer a faixa de CEPs. **Mesmo gatilho e mesma solução do rate limit da landing**, e devem ser resolvidos juntos. O cache de 500 entradas absorve repetição, não varredura | `pd-14`, 12/09/2026 |
...
| **`pd-19`** | 🚀 **Publicar** — a próxima a abrir, fora da ordem acima. Materializa o [ADR-0020](../06-decisions/ADR/0020-hosting-do-piloto-railway-e-cloudflare.md): os três serviços no Railway com `prisma migrate deploy` como pre-deploy command, o `_redirects` do app web no Cloudflare Pages, DNS e certificados, as variáveis por serviço, a `/precos` fora do menu e do sitemap até o censo (E9), e uma **página `/privacidade` na landing** que resolve de uma vez os itens **4, 12 e 15** da intervenção manual (canal de contato real, base legal do cadastro com destino para o link do `/cadastro`, e a declaração de que os dados ficam nos Estados Unidos). Dispara, e por isso resolve nela (§3.2), os itens de vigilância com gatilho "deploy público": **rate limit** da captura e do `/postal-codes`, **serviço de observabilidade** (ADR-0006 #8), **CSP do app web** e o **`[param].html` do export estático**. Landing e API primeiro; o app web depois | ⏳ **Conta no Railway — o Victor cria quando a publicação estiver a dias** (ADR-0020, E8; 13/09/2026: *"depois crio a conta na Railway"*). Também dele: colar o `JWT_SECRET`, trocar os nameservers no Registro.br, promover `develop` → `master` |
