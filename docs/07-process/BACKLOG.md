...
| Item | Detalhe | Origem |
|---|---|---|
| 🔴 **`MVP_SCOPE` contradiz o ADR-0004 sobre o que é o MVP** | **Medido em 06/09/2026**, lendo a working tree: [`01-product/MVP_SCOPE.md`](../01-product/MVP_SCOPE.md) §"Fora do escopo" (linhas 83-97) lista **"Marketplace de produtos e catálogo de Pet Shops" como Fase 4**, justificado por "monetização via comércio — pós-fidelização", e descreve um MVP centrado em Tutores e Pets. O [ADR-0004](../06-decisions/ADR/0004-arquitetura-mvp-marketplace.md), **aceito em 03/09/2026**, faz exatamente o oposto: o marketplace hiperlocal **é** o produto inicial, e o ADR declara que substitui o desenho anterior. Os dois documentos estão vivos e se contradizem no ponto mais central do produto. **Impacto:** o `PROJECT_STATE.md` manda iniciar a implementação "a partir de `MVP_SCOPE`" — quem seguir essa instrução literalmente constrói o produto errado. Ambos os arquivos (`MVP_SCOPE` e `PRODUCT_ROADMAP`) já se declaram `outdated` no frontmatter, o que sinaliza a defasagem mas não impede o uso. **Trabalho:** reescrever `MVP_SCOPE` e `PRODUCT_ROADMAP` sob o ADR-0004 e devolvê-los a `draft`/`stable` — e ver o item abaixo, que estende a mesma contradição a outros três documentos de `01-product` | Leitura cruzada de `MVP_SCOPE.md:83-97` × ADR-0004, 06/09/2026 |
| **`CAPABILITIES`, `FEATURE_CATALOG`, `USER_JOURNEYS` e `SUCCESS_METRICS` também dão o "Vida do Pet" como MVP, e sem aviso de defasagem** | **Medido em 08/09/2026**, lendo os arquivos na working tree. É o mesmo defeito do item acima com alcance maior, e com um agravante: os quatro estão `status: draft` — **não** `outdated` —, em v1.0 de **27/06/2026**, anteriores ao ADR-0004; ou seja, **leem-se como válidos**. Evidências: [`CAPABILITIES.md`](../01-product/CAPABILITIES.md) põe Marketplace como C11, **Fase 4**, "capacidade-satélite, não núcleo"; [`FEATURE_CATALOG.md`](../01-product/FEATURE_CATALOG.md) lista a Fase 1 como Timeline, Carteira Digital e Lembretes; [`USER_JOURNEYS.md`](../01-product/USER_JOURNEYS.md) descreve as seis jornadas do MVP (J1–J6) **todas** no produto anterior e dispara eventos que o `DOMAIN_MODEL` v2.0 não tem (`tutor.linked_to_pet`, `timeline.event.created`); [`SUCCESS_METRICS.md`](../00-foundation/SUCCESS_METRICS.md) define o North Star como "tutores com a vida do pet centralizada" e mede Timeline/Carteira, quando o [`BUSINESS_MODEL`](../00-foundation/BUSINESS_MODEL.md) v2.0 (autoridade maior) diz que a métrica-mãe da fase 1 é **pedido recorrente por tutor**. **Diagnóstico revisado em 10/09/2026 — não é 🔴:** o registro de 08/09 dizia que o `USER_JOURNEYS` "pauta o spike-gate" e por isso contaminaria a decisão de stack. Reconferido: o critério **autoritativo** do spike está em [`TECHNOLOGY_STACK`](../02-architecture/TECHNOLOGY_STACK.md) §"Spike-gate do cliente universal" e no `PROJECT_STATE` §"Próxima Atividade", e **ambos já estão corretos** (comparador, checkout, painel do lojista). A seção "insumo do spike-gate" do `USER_JOURNEYS` contradiz esses dois em autoridade menor — risco real de um agente seguir o documento errado, mas não é medição errada garantida. **Trabalho:** reescrever os quatro sob o ADR-0004, junto da reescrita do item acima | Leitura dos arquivos na working tree, 08/09/2026; diagnóstico revisado em 10/09/2026 |
